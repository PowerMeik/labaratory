...
int main(int argc,char *argv[])
{
 ...
 if (argc != 4) {
    ...
    int iteration_cnt = std::stoi(argv[1]);
        //std::cout<<iters<<std::endl;
    int  net_len = std::stoi(argv[2]);
    ...
    unsigned int blocks = (net_len+threads-1)/threads;

        set_border<<<blocksPerGrid, threadsPerBlock>>>(net, net_len, lu, ld, ru, rd);
