...
int main(int argc, char** argv)
{
	...
	while(iter < maxIter && error > minError)
	{
		...
		else
		{
			...
			{
				calculateMatrix<<<block_for_interpolate, dim_for_interpolate, 0, stream>>>(deviceMatrixAPtr, deviceMatrixBPtr, size);
				calculateMatrix<<<block_for_interpolate, dim_for_interpolate, 0, stream>>>(deviceMatrixBPtr, deviceMatrixAPtr, size);
			}
			// Расчитываем ошибку каждую сотую итерацию
			getErrorMatrix<<<threads * blocks * blocks, threads,  0, stream>>>(deviceMatrixAPtr, deviceMatrixBPtr, errorMatrix);
